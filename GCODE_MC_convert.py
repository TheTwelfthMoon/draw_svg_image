...
class convert_gcode2motorcortex():
    @staticmethod
    def sign(a):
        if a < 0:
            return -1
    ...
    def circleToThreePoint(point1, point2, radius, direction):
        ...
        if (convert_gcode2motorcortex.sign(angle1) == direction):
            center = [x01, y01]
        else: 
            center = [x02, y02]
        ...
        if (convert_gcode2motorcortex.sign(angle3) == direction):
            point3 = p1
        else:
            point3 = p2
            ...
            try:
                if (line.command == ('G', 21)):
                    ...
                    if (currentPath["type"] != 'L'):
                        points.append(currentPath)
                        currentPath = {"type": 'L', "points":[]}
                    # currentPath["points"].append([line.params['X'] * scale, line.params["Y"] * scale, 0] + to_radians([0, 0.0, 180.0]))
                    ...
                    if len(lastPoint) == 3:
                        ...
                        point3 = convert_gcode2motorcortex.circleToThreePoint(lastPoint[:2], [(line.get_param('X')*scale), (line.get_param('Y')*scale)], line.get_param('R') * scale, 1)
